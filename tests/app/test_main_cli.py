...
from app.bootstrap.entry import parse_args, resolve_ui_mode
...
def test_format_peer_reject_notice_includes_reason_and_node_ip():
    ctx = SimpleNamespace(
        get_node=lambda node_id: SimpleNamespace(name="B", ip="10.0.0.20") if node_id == "B" else None
    )
    ...
    assert message == "B(10.0.0.20) 노드가 연결을 거부했습니다. 사유: 상대 노드 목록에 현재 PC 정보가 없습니다."
