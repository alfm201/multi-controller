...
def test_refresh_updates_summary_and_delegates_to_layout_editor():
    ...
    assert window._vars["headline"].get() == "B PC를 제어 중입니다."
    assert window._vars["summary"].get() == "연결된 PC 1 / 1"
    assert (
        window._vars["hint"].get()
...
def test_reload_and_clear_actions_update_runtime_message():
    ctx = _layout_ctx()
    ...
    assert window._vars["message"].get() == "B PC로 전환을 요청했습니다."
