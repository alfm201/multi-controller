...
from app.logging.app_logging import TAG_HOTKEY, tag_message
...
class HotkeyMatcher:
    ...
    def on_press(self, key_str: Optional[str]) -> bool:
        ...
        logging.info(tag_message(TAG_HOTKEY, "%s matched"), self._name)
        try:
            self._callback()
        except Exception as exc:
            logging.warning(tag_message(TAG_HOTKEY, "%s callback raised: %s"), self._name, exc)
        return True
...
class TargetCycler:
    ...
    def _step(self, offset: int) -> Optional[str]:
        ...
        if not targets:
            logging.info(tag_message(TAG_HOTKEY, "cycle skipped: no peers available"))
            return None
        ...
        logging.info(tag_message(TAG_HOTKEY, "cycle %s -> %s"), current, next_id)
        if callable(self.before_select):
            try:
                self.before_select(next_id)
            except Exception as exc:
                logging.warning(tag_message(TAG_HOTKEY, "cycle pre-select hook failed: %s"), exc)
        if self.coord_client is not None:
            try:
                ...
                logging.warning(tag_message(TAG_HOTKEY, "cycle request failed: %s"), exc)
        else:
            if next_id == self_id:
