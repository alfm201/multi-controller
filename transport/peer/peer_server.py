...
from transport.peer.frames import encode_frame
...
from transport.peer.peer_reject import REJECT_REASON_UNKNOWN_NODE, make_peer_reject
...
class PeerServer:
    ...
    def start(self):
        ...
        logging.info(
            tag_message(TAG_PEER, "server listening 0.0.0.0:%s self=%s"),
    ...
    def stop(self):
        ...
        if self._thread is not None:
            ...
            if self._thread.is_alive():
                logging.warning(tag_message(TAG_PEER, "accept thread did not exit in time"))
    ...
    def _handshake_inbound(self, sock, addr):
        ...
        try:
            ...
            logging.info(tag_message(TAG_PEER, "inbound handshake failed from %s: %s"), addr, exc)
            try:
                sock.close()
            except OSError:
                pass
        ...
        if peer_hello.bootstrap:
            ...
            try:
                ...
                logging.warning(tag_message(TAG_PEER, "bootstrap handler failed from %s: %s"), addr, exc)
            finally:
                try:
                    sock.close()
                except OSError:
        ...
        if self.ctx.get_node(peer_hello.node_id) is None:
            if hasattr(self.ctx, "is_pending_join_node") and self.ctx.is_pending_join_node(
                ...
                logging.info(
                    tag_message(TAG_PEER, "handshake allowed for pending join node_id=%r from %s"),
                    peer_hello.node_id,
                    addr,
                )
            else:
                logging.info(
                    tag_message(TAG_PEER, "handshake rejected unknown node_id=%r from %s"),
                    peer_hello.node_id,
                    addr,
                )
                try:
        ...
        if not self.registry.bind(peer_hello.node_id, conn, notify=False):
            logging.info(
                tag_message(TAG_PEER, "handshake duplicate peer=%s from %s, closing loser"),
                peer_hello.node_id,
                addr,
            )
            conn.close()
        ...
        logging.info(tag_message(TAG_PEER, "accepted peer=%s from %s"), peer_hello.node_id, addr)
