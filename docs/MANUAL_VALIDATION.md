...
4. coordinator, active target, peer 연결 상태가 기대대로 보이는지 확인
...
1. GUI에서 편집 모드를 켠다
2. 대상 PC를 선택한 뒤 `모니터 맵 편집`을 열어 논리/물리 배치를 다르게 입력한다
3. `검증`으로 문법과 display id 일치 여부를 먼저 확인한다
4. `적용` 후 다른 장비 GUI에도 모니터 경계 표시가 즉시 반영되는지 본다
5. 논리 배치의 좌측하단, 상단, 우측 경계를 천천히 넘어가며 어떤 PC/모니터로 이어지는지 확인한다
6. 창 제목 표시줄 드래그, 파일 드래그, 영역 선택처럼 버튼 홀드 상태로 같은 경계를 넘겨 본다
7. 경계를 넘은 직후 즉시 반대 방향으로 떨리듯 되돌아가지 않는지 확인한다
