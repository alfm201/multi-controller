...
from app.logging.app_logging import TAG_ERROR, tag_message
...
def _run_cleanup_actions(cleanup_actions) -> None:
    for action in cleanup_actions:
        try:
            ...
            logging.warning(tag_message(TAG_ERROR, "cleanup action failed during exception handling: %s"), exc)
...
def _show_qt_error_dialog(title: str, message: str) -> bool:
    ...
    try:
        ...
        logging.debug(tag_message(TAG_ERROR, "Qt error dialog failed: %s"), exc)
        return False
...
def _show_windows_error_dialog(title: str, message: str) -> bool:
    ...
    try:
        ...
        logging.debug(tag_message(TAG_ERROR, "MessageBoxW failed: %s"), exc)
        return False
...
def install_unhandled_exception_handler(
    *,
    ...
    def _handle_exception(exc_type, exc_value, traceback_obj, *, previous_hook=None, thread_args=None) -> None:
        ...
        try:
            logging.critical(
                tag_message(TAG_ERROR, "unhandled exception"),
        ...
        if should_show:
            try:
                ...
                logging.debug(tag_message(TAG_ERROR, "failed to show user-friendly error dialog: %s"), exc)
