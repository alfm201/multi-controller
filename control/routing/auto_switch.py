...
from control.routing.edge_routing import EdgeRoutingResolver
from control.routing.remote_pointer import ActiveRemotePointer
from app.logging.app_logging import TAG_SWITCH, tag_message
from model.display.display import get_virtual_screen_bounds
...
class AutoTargetSwitcher:
    ...
    def process(self, event):
        ...
        try:
            ...
            logging.warning(tag_message(TAG_SWITCH, "failed to process event: %s"), exc)
            return event
    ...
    def on_router_state_change(self, state: str, node_id: str | None) -> None:
        ...
        if callable(self._actual_pointer_provider):
            try:
                ...
                logging.debug(tag_message(TAG_SWITCH, "failed to read local anchor pointer: %s"), exc)
                anchor_local = None
        anchor_event = None
