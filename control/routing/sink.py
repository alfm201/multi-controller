...
class InputSink:
    ...
    def handle(self, peer_id, event):
        if not self._is_authorized(peer_id):
            logging.debug(
                tag_message(TAG_SINK, "dropping unauthorized input from=%s holder=%s kind=%s"),
                peer_id,
        ...
        if controller_id is not None:
            ...
            if callable(prepare_remote):
                try:
                    ...
                    logging.debug(tag_message(TAG_SINK, "prepare_remote_control failed: %s"), exc)
        else:
            ...
            if callable(end_remote):
                try:
                    ...
                    logging.debug(tag_message(TAG_SINK, "end_remote_control failed: %s"), exc)
