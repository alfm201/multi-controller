...
from app.logging.app_logging import TAG_ROUTER, tag_message
...
class InputRouter:
    ...
    def _swap_state(self, state, requested_target_id, active_target_id, reason=None):
        ...
        if reason:
            logging.info(
                tag_message(TAG_ROUTER, "state %s:%s -> %s:%s (%s)"),
                prev_state,
                prev_target,
                state,
                next_target,
            ...
            logging.info(
                tag_message(TAG_ROUTER, "state %s:%s -> %s:%s"),
                prev_state,
                prev_target,
                state,
                next_target,
    ...
    def _reset_local_held_state(self):
        with self._lock:
            ...
            self._pending_handoff_entries.clear()
        logging.info(tag_message(TAG_ROUTER, "cleared %s held local input(s) on transition"), held_count)
