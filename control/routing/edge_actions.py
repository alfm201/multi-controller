"""Edge action execution for auto-switch control.routing."""
...
class EdgeActionExecutor:
    ...
    def apply_route(self, transition: EdgeTransition, route):
        ...
        if hasattr(self.router, "has_pressed_mouse_buttons") and self.router.has_pressed_mouse_buttons():
            self._log_action_once(
                ...
                tag_message(TAG_SWITCH, "target switch blocked while dragging on %s:%s via %s edge"),
        ...
        if destination.node_id == self.ctx.self_node.node_id:
            ...
            self.clear_target()
            log_detail(
                tag_message(TAG_SWITCH, "%s:%s -> self:%s via %s edge"),
                frame.current_node_id,
                frame.current_display_id,
                destination.display_id,
                transition.direction,
            ...
            log_detail(
                tag_message(TAG_SWITCH, "%s:%s -> %s:%s via %s edge"),
                frame.current_node_id,
                frame.current_display_id,
                destination.node_id,
                destination.display_id,
    ...
    def _apply_block(
        ...
        *,
        ...
        if frame.current_node_id == self.ctx.self_node.node_id:
            ...
            if route.reason == "offline-target" and route.destination is not None:
                self._log_action_once(
                    ...
                    tag_message(TAG_SWITCH, "self offline edge blocked on %s via %s edge (target=%s)"),
                    frame.current_display_id,
                    transition.direction,
                    route.destination.node_id,
                )
                ...
                self._log_action_once(
                    ...
                    tag_message(TAG_SWITCH, "self logical gap blocked on %s via %s edge (logical=%s)"),
                    frame.current_display_id,
                    transition.direction,
                ...
                self._log_action_once(
                    ...
                    tag_message(TAG_SWITCH, "remote switch disabled on %s via %s edge (target=%s:%s)"),
                    frame.current_display_id,
                    transition.direction,
                    route.destination.node_id,
                    route.destination.display_id,
                ...
                self._log_action_once(
                    ...
                    tag_message(TAG_SWITCH, "self dead edge blocked on %s via %s edge"),
                    frame.current_display_id,
                    transition.direction,
                )
        ...
        self._log_action_once(
            ...
            (
                tag_message(TAG_SWITCH, "target logical gap blocked on %s:%s via %s edge (logical=%s)")
                if route.reason == "target-logical-gap"
                else tag_message(TAG_SWITCH, "remote switch disabled on %s:%s via %s edge (target=%s:%s)")
                if route.reason == "remote-switch-disabled" and route.destination is not None
                else tag_message(TAG_SWITCH, "target edge blocked on %s:%s via %s edge")
            ),
            frame.current_node_id,
            frame.current_display_id,
            transition.direction,
    ...
    def _apply_internal_warp(
        ...
        *,
        ...
        if frame.current_node_id == self.ctx.self_node.node_id:
            ...
            log_detail(
                tag_message(TAG_SWITCH, "self internal display %s -> %s via %s edge"),
                frame.current_display_id,
                destination.display_id,
                transition.direction,
            )
        ...
        log_detail(
            tag_message(TAG_SWITCH, "target internal display %s:%s -> %s via %s edge"),
            frame.current_node_id,
            frame.current_display_id,
            destination.display_id,
            transition.direction,
    ...
    def _repair_initial_local_block_leak(
        ...
        self._log_action_once(
            ...
            tag_message(TAG_SWITCH, "self block fallback warp on %s via %s edge after clip could not confine pointer"),
            transition.frame.current_display_id,
            transition.direction,
        )
        return self._repair_local_hold_leak(
